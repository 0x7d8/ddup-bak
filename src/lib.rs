...
impl Archive {
    ...
    pub fn open(path: &str) -> Result<Self, std::io::Error> {
        ...
        println!("Entries offset: {}", entries_offset);
    ...
    fn encode_entry(
        ...
        if metadata.is_file() {
            ...
            if let Some(entries) = entries {
                entries.push(Entry::Directory(dir_entry));
            } else {
                self.entries.push(Entry::Directory(dir_entry));
            }
            ...
            if let Some(entries) = entries {
                entries.push(Entry::Symlink(link_entry));
            } else {
                self.entries.push(Entry::Symlink(link_entry));
            }
        }
